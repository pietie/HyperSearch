...
namespace HyperSearch
{
    public static class KbHook
    {
        ...
        private const int KEYEVENTF_DOWN = 0;
        private const int KEYEVENTF_UP = 0x2;
        ...
        public static void SendKeyDownAndUp(System.Windows.Forms.Keys key)
        {
            var keyToSend = (ushort)key;
            var extraMsgInfo = GetMessageExtraInfo();

            INPUT[] inputsKeyDown = new INPUT[]
                            {
                                new INPUT
                                {
                                    type = INPUT_KEYBOARD,
                                    u = new InputUnion
                                    {
                                        ...
                                        {
                                            wVk = keyToSend,
                                            dwFlags = KEYEVENTF_DOWN,
                                            dwExtraInfo = extraMsgInfo,
                                        }
                                    }
                                }
                            };

            var retCode = SendInput((uint)inputsKeyDown.Length, inputsKeyDown, Marshal.SizeOf(typeof(INPUT)));

            INPUT[] inputsKeyUp = new INPUT[]
                            {
                                new INPUT
                                {
                                    type = INPUT_KEYBOARD,
            ...
            retCode = SendInput((uint)inputsKeyUp.Length, inputsKeyUp, Marshal.SizeOf(typeof(INPUT)));
        }
                    ...
                    {
                        {
                            ...
                            {
                                LastTriggerWMDownStart = DateTime.Now;
                            }
                            ...
                            {
                                LastTriggerWMDownStart = null;
                                OnTriggerKeyHit(vk, null);
                            }


                            //return new IntPtr(1); // ignore original!
                        }
                    }
                }
...
// http://stackoverflow.com/a/13109774
